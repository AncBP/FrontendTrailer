...
const ManoDeObra = () => {
  ...
  const API_USER = 'https://api.trailers.trailersdelcaribe.net/api/user';
  ...
  const [contratista, setProveedores] = useState([]);
  ...
  const fetchData = async () => {
    try {
      const manoObraParams = showOnlyActive
        ? { params: { filter: 'Activo' } }
        : {};

      const [resManoObra, resContratista] = await Promise.all([
        axios.get(API_URL, manoObraParams),
        axios.get(API_USER),
      ]);

      setManoDeObra(resManoObra.data);
      setProveedores(resContratista.data.data || resContratista.data);
    } catch (error) {
  ...
  // Filtrado
  const filtrados = (manoDeObra || []).filter(d =>
    d.name?.toLowerCase().includes(busqueda.toLowerCase()) ||
    d.type?.toLowerCase().includes(busqueda.toLowerCase()) ||
    (
      (d.contractor?.firstName + ' ' + d.contractor?.lastName)
        ?.toLowerCase()
  ...
  const handleGuardar = async () => {
    try {
      ...
      if (modo === 'editar') {
        ...
        await axios.patch(
          `${API_URL}/${nuevoManoDeObra.idManPower}`,
          payload
        );
        toast.success('Mano de obra actualizado correctamente');
      } else {
        await axios.post(API_URL, payload);
        toast.success('Mano de obra creado correctamente');
      }
  ...
  // FIX: Corrected the handleEliminar function
  const handleEliminar = async (id) => {
    // Add validation to ensure id exists
    if (!id) {
      toast.error('ID no válido para eliminar');
    ...
    try {
      console.log('Eliminando ID:', id); // Debug log
      await axios.delete(`${API_URL}/${id}`);
      toast.success('Mano de obra eliminado correctamente');
      await fetchData();
    } catch (err) {
      console.error('Error al eliminar:', err);
      toast.error(
        err.response?.data?.message ||
        'Ocurrió un error al eliminar la mano de obra.'
      );
    }
  };
